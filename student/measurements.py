...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        ############
        pos_veh = np.ones((4,1))
        pos_veh[0:3] = x[0:3] # homogeneous coordinates
        pos_sens = self.veh_to_sens * pos_veh # transfrom from vehicle to sensor coordinates

        # calculate angle of target 
        if pos_sens[0] > 0: # avoid division by 0 in angle calculation
            alpha = np.arctan(pos_sens[1]/pos_sens[0])
            # check if in FoV or not
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            # - return h(x)
            ############
            # transform position from vehicle to camera coordinates
            pos_veh = np.ones((4,1))
            pos_veh[0:3] = x[0:3]
            ...
            hx = np.zeros((2,1))
            if pos_sens[0] == 0:
                raise NameError('h(x) not defined for pos_sens[0]=0!')
            else:

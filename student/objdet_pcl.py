...
def load_range_image(frame, lidar_name):
    ...
    # ri contains 4 layers in third dimension: range, intensity, elongation, is_in_no_label_zone
    return ri
...
def show_pcl(pcl):
    ...
    pcd.points = o3d.utility.Vector3dVector(pcl[:,:3])  # transform point clouds (x,y,z) cartesian coordinates to vectors
    ...
    vis.register_key_callback(262, next_pcd_cb)
    vis.update_renderer()
    vis.poll_events()   
    vis.run()
    #######
...
def show_range_image(frame, lidar_name):
    ...
    percentile_values = np.percentile(ri_intensity, [1, 99])
    ri_intensity[ri_intensity>percentile_values[1]] = percentile_values[1]
    img_intensity = ri_intensity * 255 / (np.amax(ri_intensity) - np.amin(ri_intensity))
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / bev_discret_y) - (configs.bev_width + 1) / 2)
    ...
    _, idx_height_unique, counts = np.unique(lidar_pcl_cpy[:,:2], axis=0, return_index=True, return_counts=True)
    ...
    img_intensity = intensity_map * 255 / np.amax(intensity_map)
    img_intensity = img_intensity.astype(np.uint8)
    cv2.namedWindow('img_intensity',cv2.WINDOW_NORMAL)
    while (1):
    ...
    img_height = height_map * 255 / np.amax(intensity_map)
